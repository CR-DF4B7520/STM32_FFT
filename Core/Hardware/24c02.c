...
HAL_StatusTypeDef   EP24C_WriteOneByte(uint16_t memAddress, uint8_t byteData, bool DMA)
{
    ...
    if (DMA == false)
        result = HAL_I2C_Mem_Write(&I2C_HANDLE, DEV_ADDR_24CXX, memAddress,EP24C_MEMADD_SIZE, &byteData, 1, EP24C_TIMEOUT);
    else
        result = HAL_I2C_Mem_Write_DMA(&I2C_HANDLE, DEV_ADDR_24CXX, memAddress,EP24C_MEMADD_SIZE, &byteData, 1);
